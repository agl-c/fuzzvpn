...
for conf_file in /tcp-fuzzedconfigs/*.conf; do
    echo "Processing $conf_file..."
    openvpn --config "$conf_file" &> "/tcp-fuzzedconfigs/$(basename "$conf_file" .conf).log" &
    pid=$!
    sleep 3
    kill $pid
    wait $pid 2>/dev/null
...
for conf_file in /tcp-fuzzedconfigs/*.ovpn; do
    echo "Processing $conf_file..."
    openvpn --config "$conf_file" &> "/tcp-fuzzedconfigs/$(basename "$conf_file" .ovpn).log" &
    pid=$!
    sleep 3
    kill $pid
    wait $pid 2>/dev/null
