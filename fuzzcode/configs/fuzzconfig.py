...
for variable_name in ['port', 'keepalive', 'server']:
    for fuzz_way in ['zero', 'large']:
        output_file = output_dir + 'server-raw-' + variable_name + '-' + fuzz_way + '.conf'
...
for variable_name in ['port', 'keepalive', 'server',
 ...
 'data-ciphers-fallback', 'topology', 'user', 'group']:
    output_file = output_dir + 'server-raw-' + variable_name + '-add_null.conf'
    modify_variable_in_file(input_file, output_file, variable_name, 'add_null')


variable_name = "port"
output_file = output_dir + 'server-raw-' + variable_name + '-malport.conf'
modify_variable_in_file(input_file, output_file, variable_name, 'malport')
# 6+14+1=21 

# below, we generate fuzzed client configs 
input_file = 'client1-raw-fuzz.ovpn'

# numeric values
for variable_name in ['lport', 'remote']:
    for fuzz_way in ['zero', 'large']:
        output_file = output_dir + 'client1-raw-' + variable_name + '-' + fuzz_way + '.ovpn'
        modify_variable_in_file(input_file, output_file, variable_name, fuzz_way)
...
for variable_name in ['lport', 'remote',
 'proto', 'dev', 'allow-compression', 'data-ciphers-fallback', 'remote-cert-tls']:
    output_file = output_dir + 'client1-raw-' + variable_name + '-add_null.ovpn'
    modify_variable_in_file(input_file, output_file, variable_name, 'add_null')
 
variable_name = "lport"
output_file = output_dir + 'client1-raw-' + variable_name + '-malport.ovpn'
modify_variable_in_file(input_file, output_file, variable_name, 'malport')
