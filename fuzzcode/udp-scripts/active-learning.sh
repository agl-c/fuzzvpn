#!/bin/bash
# This script is used to run fuzzing experiments on the OpenVPN UDP proxy with active learning to construct the MSC 
log_dir="/udp-active-learning-logs"   
...
run_fuzz(){
    ...
    fuzz_log="$log_dir/$fuzzway-$allowed_control_v1_num-udpproxy.log"
    ...
    # server side tcpdump
    ser_pcap_file="$log_dir/$fuzzway-$allowed_control_v1_num-ser-raw.pcap"
    ...
    cli_pcap_file="$log_dir/$fuzzway-$allowed_control_v1_num-cli-raw.pcap"
    ...
    server_log="$log_dir/$fuzzway-$allowed_control_v1_num-server-raw"
    server_err="$log_dir/$fuzzway-$allowed_control_v1_num-err-server-raw"
    ...
    client_log="$log_dir/$fuzzway-$allowed_control_v1_num-client-raw"
    client_err="$log_dir/$fuzzway-$allowed_control_v1_num-err-client-raw"
...
for allowed_control_v1_num in $(seq 1 9); do
